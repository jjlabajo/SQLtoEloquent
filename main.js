...
function where(value, w, pre = "where"){
    ...
    if(typeof parts[1] == 'undefined' && typeof w[` ${parts[0].trim()}`] !== 'undefined'){
        grouped_value = w[` ${parts[0]}`].trim()
        if(/between /g.test(grouped_value) && grouped_value.split(" ").length < 6){
            grouped_value = grouped_value.replace(/\(/g, "")
            grouped_value = grouped_value.replace(/\)/g, "")
            grouped_parts = grouped_value.split(" ").map(function(x){return x.trim()})
            return `->${pre}Between("${grouped_parts[0]}", [${grouped_parts[2] || ""}, ${grouped_parts[4] || ""}])`
        }
        return `->${pre}(DB::raw("${grouped_value}"))`
    }else if(typeof parts[1] == 'undefined'){
        console.log(parts)
        return `->${pre}(DB::raw("${value.trim()}"))`
    }
